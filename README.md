...
每次摇骰子：随机抽 **3 种手法**（不重复）× 每种配 **1–2 个随机词**（40 个词库，如钟表、灯塔、蜂巢…）× **1 条随机约束**（16 条，如"只用 10 个字以内解释清楚""完全不用电"），生成一段可以直接复制的**灵感提示语**。

## 两种模式

**🧊 离线模式（默认，零成本）**
- 摇出灵感提示语 + "用词 → 三步跳跃"的思路演示（属性 → 模式 → 映射）
- 点「复制提示语」，粘贴给任意 AI（或自己顺着思路想）

**⚡ 在线模式（可选）**
- 填入 API 地址、Key、模型名（OpenAI 兼容接口，如 DeepSeek / OpenAI / Ollama / 硅基流动等）
- 摇骰子直接让 AI 出 3 个点子
- **Key 只存在你自己的浏览器 localStorage 里**，页面代码不收集、不上传任何东西

## 本地使用

1. 下载/克隆本项目，找到 `inspiration-dice` 文件夹
2. 双击 `index.html` 即可使用（无需任何环境）
3. 在线模式需要先点「⚡ 在线模式」→ 填配置 → 「保存到本地」
...
- 纯静态页面，无任何外部请求（离线模式完全断网可用）
- API Key 仅保存在浏览器 localStorage，只在你点击摇骰子时发给你自己填的 API 地址
- 可随时点「清除」抹掉本地配置
...
├── script.js     # 交互：摇骰子、渲染、复制、在线模式 AI 调用
├── README.md
└── LICENSE       # MIT
```
